...
## Forge's Approach: Contracts + Verification + Attestation

Atoryn Forge solves the governance gap using a deterministic contract-and-evidence protocol.
...
2. **Verification**: After the AI implements the code, Forge analyzes the exact Git object tree diff. It checks the modifications against the contract's scope rules, safely runs the verification commands, and ensures no unauthorized workspace mutations occurred during verification.
3. **Attestation**: Forge produces a cryptographically signed JSON evidence record. This record irrefutably proves whether the AI's changes adhered to the contract and whether the verification checks passed.
